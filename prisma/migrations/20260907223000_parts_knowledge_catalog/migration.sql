-- Extend the existing local alias catalog with provider-aware knowledge fields.
ALTER TABLE "GenericArticleAlias"
  ADD COLUMN IF NOT EXISTS "aliasType" VARCHAR(40) NOT NULL DEFAULT 'SYNONYM',
  ADD COLUMN IF NOT EXISTS "language" VARCHAR(8) NOT NULL DEFAULT 'uk',
  ADD COLUMN IF NOT EXISTS "provider" VARCHAR(64) NOT NULL DEFAULT '',
  ADD COLUMN IF NOT EXISTS "axisHint" VARCHAR(16),
  ADD COLUMN IF NOT EXISTS "sideHint" VARCHAR(16),
  ADD COLUMN IF NOT EXISTS "subPositionHint" VARCHAR(16),
  ADD COLUMN IF NOT EXISTS "sourceVersion" VARCHAR(160),
  ADD COLUMN IF NOT EXISTS "status" "CatalogEntityStatus" NOT NULL DEFAULT 'DRAFT',
  ADD COLUMN IF NOT EXISTS "usageCount" INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS "approvedByUserId" VARCHAR(128),
  ...
  ADD COLUMN IF NOT EXISTS "identityKey" VARCHAR(128);

UPDATE "GenericArticleAlias"
SET "identityKey" = md5(concat('LEGACY', chr(31), "id"))
...
CREATE INDEX "generic_article_alias_lookup_idx"
  ON "GenericArticleAlias"("aliasNormalized", "status");
CREATE INDEX "generic_article_alias_article_idx"
  ON "GenericArticleAlias"("genericArticleId", "status");
CREATE INDEX "generic_article_alias_provider_idx"
  ON "GenericArticleAlias"("provider", "status");
