...
async function authSmoke() {
  ...
  await expectServiceError(() => verifyServiceAccessToken(valid, "lead:submit", config, fetcher), 403, "SERVICE_SCOPE_DENIED");
  const wrongAudience = await signer.token({ ...baseClaims, aud: "https://vercel.com/other" });
  await expectServiceError(() => verifyServiceAccessToken(wrongAudience, "vehicle:resolve", config, fetcher), 401, "SERVICE_AUTH_INVALID");
  const expired = await signer.token({ ...baseClaims, exp: now - 60 });
  await expectServiceError(() => verifyServiceAccessToken(expired, "vehicle:resolve", config, fetcher), 401, "SERVICE_AUTH_INVALID");
  const unknownProject = await signer.token({ ...baseClaims, sub: "owner:turbo-lev:project:unknown:environment:production", project: "unknown" });
  await expectServiceError(() => verifyServiceAccessToken(unknownProject, "vehicle:resolve", config, fetcher), 403, "SERVICE_PRINCIPAL_DENIED");
  const preview = await signer.token({ ...baseClaims, sub: "owner:turbo-lev:project:turbolev-web:environment:preview", environment: "preview" });
  await expectServiceError(() => verifyServiceAccessToken(preview, "vehicle:resolve", config, fetcher), 403, "SERVICE_PRINCIPAL_DENIED");
  const tamperedParts = valid.split(".");
  const tampered = `${tamperedParts[0]}.${b64({ ...baseClaims, project: "tampered" })}.${tamperedParts[2]}`;
  await expectServiceError(() => verifyServiceAccessToken(tampered, "vehicle:resolve", config, fetcher), 401, "SERVICE_AUTH_INVALID");
  await expectServiceError(() => verifyServiceAccessToken(valid, "vehicle:resolve", { ...config, policies: [] }, fetcher), 503, "SERVICE_AUTH_NOT_CONFIGURED");
}
...
async function guardDbSmoke() {
  ...
  const service = { subject: "owner:turbo-lev:project:turbolev-web:environment:production", environment: "production" as const };
  ...
  assert.equal(stableRequestFingerprint(operationKey, { b: 2, a: 1 }), stableRequestFingerprint(operationKey, { a: 1, b: 2 }));
  ...
  assert.match(correlationIdForRequest(new Request("https://crm.example.test/integration/v1/test")), /^[0-9a-f-]{36}$/i);
  ...
  const conflict = await beginIdempotentOperation({ service, operationKey, idempotencyKey: key, payload: { ...payload, mode: "VIN" } });
