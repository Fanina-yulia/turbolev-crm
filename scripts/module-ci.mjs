...
const FULL_CI_PATTERNS = [
  ...
  "scripts/ci-*.sh",
  "scripts/ci-*.sql",
  "scripts/module-ci.mjs",
  "scripts/module-scope.mjs",
  "docs/modules/module-registry.json",
  "docs/modules/ci-checks.json"
];

function fullCiReasons(files, result) {
  const reasons = [];
  const sensitive = files.filter((file) => FULL_CI_PATTERNS.some((pattern) => matches(file, pattern)));
  if (sensitive.length) reasons.push(`platform/schema/CI files: ${sensitive.join(", ")}`);
  if (result.unmatched.length) reasons.push(`unmapped production files: ${result.unmatched.join(", ")}`);
  if (result.direct.length > 1) reasons.push(`cross-module change: ${result.direct.join(", ")}`);

  const safeLite = new Set(ci.safeLiteModules || []);
  const unsafeModules = result.direct.filter((id) => !safeLite.has(id));
  if (unsafeModules.length) reasons.push(`full-gate modules: ${unsafeModules.join(", ")}`);
...
function selectedChecks(result) {
  const ids = new Set();
  for (const moduleId of result.direct) {
...
function summary(files, result, reasons, checks) {
  ...
  lines.push(
    "## Targeted stateless smoke checks",
    ...(checks.length ? checks.map((check) => `- ${check}`) : ["- none; Prisma validation and TypeScript contract check still run"]),
  );
...
const result = moduleScope(files);
...
if (args.run) {
  ...
  console.log(`\n[module-ci] ${checks.length} targeted stateless smoke check(s): OK`);
}
