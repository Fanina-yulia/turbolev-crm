...
export const VEHICLE_RESOLUTION_STATUSES = ["RESOLVED", "AMBIGUOUS", "PENDING", "FAILED"] as const;
export const VEHICLE_RESOLVE_INPUT_TYPES = ["VIN", "MANUAL", "SAVED_VEHICLE"] as const;
export const FITMENT_MATCH_STATES = [
  "MATCH_CONFIRMED",
  "MATCH_WITH_NOTES",
  "NEEDS_CONFIRMATION",
  "NOT_COMPATIBLE",
...
export type VehicleIntentContext = {
  categoryId?: string;
...
export type ProductFitmentCheckRequest = {
  vehicleResolutionId: string;
};
...
function normalizeVin(value: unknown) {
  const vin = requiredString(value, "vin", 17).replace(/\s+/g, "").toUpperCase();
  if (!/^[A-HJ-NPR-Z0-9]{17}$/.test(vin)) {
    throw new ContractValidationError("INVALID_VIN_FORMAT", "VIN повинен містити 17 допустимих символів.", {
      vin: "INVALID_VIN_FORMAT",
    });
  }
  return vin;
}
...
function parseManualVehicle(value: unknown): ManualVehicleInput {
  if (!isRecord(value)) {
    throw new ContractValidationError("VEHICLE_INPUT_INSUFFICIENT", "Потрібні дані автомобіля.", {
      vehicle: "REQUIRED",
    });
  }
  const year = optionalInteger(value.year, 1900, 2100);
  const make = requiredString(value.make, "vehicle.make", 80);
  const model = requiredString(value.model, "vehicle.model", 100);
  if (!year) {
    throw new ContractValidationError("VEHICLE_INPUT_INSUFFICIENT", "Потрібен коректний рік автомобіля.", {
      "vehicle.year": "INVALID_YEAR",
    });
  }
  const displacementCm3 = optionalInteger(value.displacementCm3, 1, 20000);
  return {
    make,
    model,
    year,
...
export function parseVehicleResolveRequest(value: unknown): VehicleResolveRequest {
  if (!isRecord(value)) {
    throw new ContractValidationError("INVALID_REQUEST", "Тіло запиту повинно бути JSON-об'єктом.");
  }
  const inputType = enumValue(value.inputType, VEHICLE_RESOLVE_INPUT_TYPES);
  if (!inputType) {
    throw new ContractValidationError("INVALID_REQUEST", "Некоректний inputType.", { inputType: "INVALID_ENUM" });
  }
  const context = parseContext(value.context);
  if (inputType === "VIN") return { inputType, vin: normalizeVin(value.vin), context };
  if (inputType === "MANUAL") return { inputType, vehicle: parseManualVehicle(value.vehicle), context };
  return { inputType, publicVehicleRef: validateOpaqueId(value.publicVehicleRef, "publicVehicleRef"), context };
}
...
export function parseVehicleResolveConfirmRequest(value: unknown): VehicleResolveConfirmRequest {
  ...
  if (!isRecord(value.answers)) {
    throw new ContractValidationError("INVALID_REQUEST", "Потрібні answers для підтвердження конфігурації.", {
      answers: "REQUIRED",
    });
  }
  ...
  if (!Object.keys(answers).length) {
    throw new ContractValidationError("INVALID_REQUEST", "Не передано жодного допустимого критерію підтвердження.", {
      answers: "EMPTY",
    });
  }
  return {
...
export function parseFitmentBrowseQuery(input: URLSearchParams) {
  ...
  if (!categoryId && !genericArticleId) {
    throw new ContractValidationError("INVALID_FILTER", "Потрібен categoryId або genericArticleId.");
  }
  if (categoryId && genericArticleId) {
    throw new ContractValidationError("INVALID_FILTER", "Передайте лише один основний catalog intent.");
  }
  const limit = optionalInteger(input.get("limit"), 1, 60) ?? 24;
  const sort = enumValue(input.get("sort") ?? "RELEVANCE", FITMENT_SORTS) ?? "RELEVANCE";
  return {
    categoryId,
    genericArticleId,
