...
export const DEFAULT_ACCESS_ROLES: AccessRolePreset[] = [
  ...
  { code: "HEAD_OF_SALES", name: "Керівник відділу продажів", description: "Продажі, команда, клієнти та контроль воронки без фінансового адміністрування.", sortOrder: 30, grants: [
    ...
    grant(PERMISSIONS.LEADS_READ,"ALL"), grant(PERMISSIONS.LEADS_WRITE,"TEAM"), grant(PERMISSIONS.LEADS_ASSIGN,"TEAM"),
    grant(PERMISSIONS.CLIENTS_READ,"ALL"), grant(PERMISSIONS.CLIENTS_WRITE,"TEAM"), grant(PERMISSIONS.PLANNER_READ,"ALL"),
    grant(PERMISSIONS.WORK_ORDERS_READ,"ALL"), grant(PERMISSIONS.WORK_ORDERS_ESTIMATE,"TEAM"), grant(PERMISSIONS.PAYROLL_SELF_READ,"SELF"),
    grant(PERMISSIONS.PERSONNEL_READ,"TEAM"), grant(PERMISSIONS.ANALYTICS_READ,"ALL"), grant(PERMISSIONS.ANALYTICS_PERSONNEL_READ,"TEAM"),
  ]},
  ...
  { code: "STATION_MANAGER", name: "Завідувач станцією", description: "Повний операційний цикл конкретної станції без доступу до глобальної фінансової адміністрації.", sortOrder: 60, grants: [
    ...
    grant(PERMISSIONS.PARTS_READ,"LOCATION"), grant(PERMISSIONS.PROCUREMENT_READ,"LOCATION"), grant(PERMISSIONS.PAYROLL_SELF_READ,"SELF"),
    grant(PERMISSIONS.PERSONNEL_READ,"LOCATION"), grant(PERMISSIONS.ANALYTICS_READ,"LOCATION"), grant(PERMISSIONS.ANALYTICS_PERSONNEL_READ,"LOCATION"),
    grant(PERMISSIONS.WARRANTY_READ,"LOCATION"), grant(PERMISSIONS.WARRANTY_WRITE,"LOCATION"),
  ]},
  { code: "MECHANIC", name: "Автомеханік", description: "Тільки призначені роботи, виробництво та власна зарплата.", sortOrder: 70, grants: [
    grant(PERMISSIONS.OVERVIEW_READ,"LOCATION"), grant(PERMISSIONS.PLANNER_READ,"ASSIGNED"), grant(PERMISSIONS.DIAGNOSTICS_READ,"ASSIGNED"),
