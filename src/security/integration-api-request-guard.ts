...
export async function beginIdempotentOperation(input: {
  ...
  if (existing.expiresAt.getTime() <= now.getTime()) {
    await prisma.integrationIdempotencyRecord.deleteMany({ where: { id: existing.id, expiresAt: { lte: now } } });
  ...
  if (existing.lockedAt <= staleBefore) {
    const takeover = await prisma.integrationIdempotencyRecord.updateMany({
      where: { id: existing.id, state: "IN_PROGRESS", requestFingerprint, lockedAt: { lte: staleBefore } },
      data: { lockedAt: now },
    });
...
export async function completeIdempotentOperation(input: {
  ...
  const result = await getPrisma().integrationIdempotencyRecord.updateMany({
    where: { id: input.recordId, state: "IN_PROGRESS", requestFingerprint: input.requestFingerprint },
    data: {
      state: "COMPLETED",
      responseStatus,
...
export async function abandonIdempotentOperation(input: { recordId: string; requestFingerprint: string }) {
...
export async function cleanupExpiredIntegrationGuards(now = new Date(), take = 500) {
  ...
  const [idempotency, rateLimits] = await prisma.$transaction([
    prisma.integrationIdempotencyRecord.findMany({
      where: { expiresAt: { lte: now } }, select: { id: true }, take: limit, orderBy: { expiresAt: "asc" },
    }),
