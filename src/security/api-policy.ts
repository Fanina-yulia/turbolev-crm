...
const RULES: Rule[] = [
  ...
  { match: prefix("/api/diagnostics"), resolve: (method) => readWrite(method, PERMISSIONS.DIAGNOSTICS_READ, PERMISSIONS.DIAGNOSTICS_WRITE, "LOCATION", "Diagnostic requests, structured checks, media and conclusions; route handlers apply ASSIGNED/LOCATION narrowing and confirmation permission.") },
  ...
  { match: (path) => prefix("/api/client-card")(path) || prefix("/api/clients")(path) || path === "/api/clients-vehicles" || prefix("/api/vehicles")(path), resolve: (method) => readWrite(method, PERMISSIONS.CLIENTS_READ, PERMISSIONS.CLIENTS_WRITE, "TEAM", "Client and vehicle intelligence/read models.") },
