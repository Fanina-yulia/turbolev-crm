...
const RULES: Rule[] = [
  ...
  { match: prefix("/api/public/diagnostic-card-pdf"), resolve: () => ({ kind: "PUBLIC_TOKEN", note: "Diagnostic Card PDF is available only through a high-entropy, expiring and revocable share token stored as SHA-256 hash; the route returns no CRM metadata." }) },
  ...
  { match: (path) => path === "/api/blockers" || /^\/api\/blockers\/[^/]+$/.test(path), resolve: (method) => readWrite(method, PERMISSIONS.PRODUCTION_READ, PERMISSIONS.PRODUCTION_WRITE, "LOCATION", "Operational blockers are station-scoped reasons for halted work; handlers enforce source and location boundaries.",) },
  ...
  { match: exact("/api/cabinet/mechanic/repair-cases"), resolve: () => internal(PERMISSIONS.PRODUCTION_READ, "ASSIGNED", "Mechanic repair cases are grouped by WorkOrder and restricted to the caller's assigned operational cases.", true) },
  ...
  { match: exact("/api/cabinet/mechanic/execution-issues"), resolve: (method) => readWrite(method, PERMISSIONS.PRODUCTION_READ, PERMISSIONS.PRODUCTION_WRITE, "ASSIGNED", "Mechanics report execution blockers only for their own assigned work; operational staff resolve them by location.") },
  { match: exact("/api/cabinet/execution-issues"), resolve: (method) => readWrite(method, PERMISSIONS.PRODUCTION_READ, PERMISSIONS.PRODUCTION_WRITE, "LOCATION", "Execution issue decisions are station-scoped administrative production actions.") },
  { match: (path) => /^\/api\/cabinet\/execution-issues\/[^/]+\/comments$/.test(path), resolve: (method) => readWrite(method, PERMISSIONS.PRODUCTION_READ, PERMISSIONS.PRODUCTION_WRITE, "LOCATION", "Execution issue comments are station-scoped workflow communication.",) },
  { match: (path) => /^\/api\/cabinet\/execution-issues\/[^/]+\/attachments\/[^/]+$/.test(path), resolve: () => internal(PERMISSIONS.PRODUCTION_READ, "LOCATION", "Execution issue attachments are protected by station scope.", true) },
  { match: prefix("/api/cabinet/mechanic/notifications"), resolve: () => internal(PERMISSIONS.OVERVIEW_READ, "SELF", "Mechanic notification history is restricted to the caller's own mechanic resource; mutations only affect owned notification events.", true) },
  ...
  { match: prefix("/api/inventory"), resolve: (method) => readWrite(method, PERMISSIONS.PROCUREMENT_READ, PERMISSIONS.PROCUREMENT_WRITE, "LOCATION", "Warehouse ledger, balances and reservations are factual station-scoped procurement operations.") },
  { match: prefix("/api/procurement"), resolve: (method) => readWrite(method, PERMISSIONS.PROCUREMENT_READ, PERMISSIONS.PROCUREMENT_WRITE, "LOCATION", "Procurement queue and nested supplier-order operations are station-scoped and require procurement read/write permissions.") },
  ...
  { match: prefix("/api/warranties"), resolve: (method) => readWrite(method, PERMISSIONS.WARRANTY_READ, PERMISSIONS.WARRANTY_WRITE, "LOCATION", "Warranty center reads completed WorkOrder service warranties and manages claims without duplicating warranty terms.") },
  { match: prefix("/api/work-journal"), resolve: () => internal(PERMISSIONS.WORK_ORDERS_READ, "ASSIGNED", "Read-only work history and protected completion-photo archive; handlers enforce factual WorkOrder scope and redact payment facts without payment permission.", true) },
  ...
  { match: (path) => /^\/api\/work-orders\/[^/]+\/invoice-pdf$/.test(path), resolve: () => internal(PERMISSIONS.WORK_ORDERS_READ, "ASSIGNED", "Server-rendered invoice PDF follows the assigned WorkOrder scope and is read-only.", true) },
  ...
  { match: (path) => /^\/api\/diagnostics\/[^/]+\/pdf(?:\/share)?$/.test(path), resolve: (method) => internal(method.toUpperCase() === "GET" ? PERMISSIONS.DIAGNOSTICS_READ : PERMISSIONS.DIAGNOSTICS_WRITE, "LOCATION", "Diagnostic Card PDF generation, retrieval and share-token creation are location-scoped; writes are strict and idempotent per card revision.", method.toUpperCase() !== "GET") },
  ...
  { match: (path) => /^\/api\/diagnostics\/[^/]+\/commercial-proposal$/.test(path), resolve: () => internal(PERMISSIONS.WORK_ORDERS_WRITE, "LOCATION", "Creating a commercial proposal from a confirmed Diagnostic Card creates WorkOrder commercial state and requires strict WorkOrder write authority.", true) },
  { match: (path) => /^\/api\/diagnostics\/[^/]+\/manual-parts(?:\/[^/]+)?$/.test(path), resolve: (method) => method.toUpperCase() === "GET" ? internal(PERMISSIONS.PARTS_READ, "LOCATION", "Manual diagnostic part recommendations are readable only inside the caller's station scope.") : internal(PERMISSIONS.WORK_ORDERS_WRITE, "LOCATION", "Manual part recommendations change WorkOrder commercial state and require strict station-scoped write authority.", true) },
  ...
  { match: exact("/api/settings/parts-knowledge"), resolve: (method) => method.toUpperCase() === "GET"
    ...
    : internal(PERMISSIONS.SETTINGS_WRITE, "ALL", "Parts knowledge seed, explicit dual-write operations, and legacy Google Sheets import.", true) },
