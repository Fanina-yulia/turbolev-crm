...
const RULES: Rule[] = [
  ...
  { match: exact("/api/client-communication/action"), resolve: () => internal(PERMISSIONS.COMMUNICATIONS_WRITE, "TEAM", "Authenticated quick client communication action records an auditable outbound-channel intent and requires strict communications write authority.", true) },
  { match: exact("/api/client-communication/status"), resolve: () => internal(PERMISSIONS.CLIENTS_READ, "TEAM", "Client communication status is an authenticated client read model and requires team-scoped client read authority.") },
