...
export function serviceOidcConfigFromEnv(): OidcConfig {
  ...
  return { issuer, audience, targetEnvironment, policies: parseServicePolicies() };
}
...
async function getJwks(config: OidcConfig, fetcher: FetchLike) {
  ...
  const discovery = await fetchJson<DiscoveryDocument>(`${issuer}/.well-known/openid-configuration`, fetcher);
