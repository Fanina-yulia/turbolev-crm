...
const QUICK_FILTERS = new Set<LeadQuickFilter>([
  "requires_attention",
  "follow_up_today",
  "junk",
]);
...
export function parseLeadStatus(value: unknown): LeadStatus {
  if (typeof value !== "string" || !LEAD_STATUSES.has(value as LeadStatus)) {
    throw new LeadDtoError("Invalid lead status");
  }
...
export function parseRejectReason(value: unknown): RejectReason | null {
  ...
  if (typeof value !== "string" || !REJECT_REASONS.has(value as RejectReason)) {
    throw new LeadDtoError("Invalid rejectReason");
  }
...
export function parseQuickFilter(value: string | null): LeadQuickFilter | undefined {
  ...
  if (!QUICK_FILTERS.has(value as LeadQuickFilter)) {
    throw new LeadDtoError("Invalid quickFilter");
  }
...
export function parseLeadPatchDto(body: unknown): LeadPatchDto {
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    throw new LeadDtoError("Request body must be an object");
  }
  ...
  if ("assignedUserId" in input) {
    output.assignedUserId = cleanNullableString(input.assignedUserId, "assignedUserId", 100);
  }
  ...
  if ("nextContactAt" in input) {
    if (input.nextContactAt === null || input.nextContactAt === "") {
      output.nextContactAt = null;
      ...
      throw new LeadDtoError("nextContactAt must be a date string, timestamp, or null");
    }
  }
