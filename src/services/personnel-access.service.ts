...
export async function getPersonnelAccessCatalog(context: AccessContext, grantedScope: AccessScopeCode | null) {
  ...
  const [staffRoles, accessRoles, locations] = await Promise.all([
    prisma.staffRole.findMany({
      where: { code: { in: allowedCodes }, isActive: true },
    ...
    prisma.serviceLocation.findMany({
      where: { isActive: true, ...(locationFilter ? { id: { in: locationFilter } } : {}) },
      select: { id: true, name: true },
      orderBy: [{ sortOrder: "asc" }, { name: "asc" }],
    }),
  ]);
  ...
  return {
    ...
    roles: staffRoles
      .filter((role) => accessByCode.has(role.code))
      .map((role) => ({
        code: role.code,
        name: role.name,
...
async function assertTargetWithinManagerScope(
  tx: Tx,
  employeeId: string,
  context: AccessContext,
  grantedScope: AccessScopeCode | null,
...
async function ensureNotLastOwner(tx: Tx, userId: string, nextRoleCode: SystemRoleCode | null, nextActive: boolean) {
  ...
  if (!ownerRole) return;
  const currentOwner = await tx.userAccessRole.findFirst({
    where: { userId, roleId: ownerRole.id, isActive: true },
    select: { id: true },
  });
  ...
  if (!otherOwners) {
    throw new PersonnelAccessError("LAST_OWNER_PROTECTED", "Не можна забрати доступ у останнього активного Власника.", 409);
  }
}

async function ensureUserForEmployee(
  tx: Tx,
  employee: { id: string; userId: string | null; firstName: string; lastName: string; email: string | null },
  roleCode: SystemRoleCode,
  locationId: string | null,
) {
  const email = String(employee.email || "").trim().toLowerCase();
  if (!email || !email.includes("@")) {
    throw new PersonnelAccessError("EMAIL_REQUIRED_FOR_CABINET", "Щоб відкрити кабінет, вкажіть коректний e-mail працівника.");
  }
  const name = `${employee.firstName} ${employee.lastName}`.trim();
  let user = employee.userId
    ? await tx.user.findUnique({ where: { id: employee.userId }, include: { employeeProfile: true } })
    : null;
  if (!user) {
  ...
  if (!user) {
    ...
    user = await tx.user.update({
      ...
      data: {
        ...
        email,
        isActive: true,
        ...(emailChanged && user.authUserId ? { authUserId: null, lastLoginAt: null, lastSeenAt: null } : {}),
      },
      include: { employeeProfile: true },
    });
  }
  ...
  if (!accessRole || !accessRole.isActive) {
    throw new PersonnelAccessError("ACCESS_ROLE_NOT_FOUND", "Для цієї посади ще не налаштована роль доступу.", 409);
  }
  await ensureNotLastOwner(tx, user.id, roleCode, true);
  const now = new Date();
  const primary = await tx.userAccessRole.findMany({
  ...
  for (const assignment of primary) {
    ...
    await tx.userAccessRole.update({
      ...
      data: { isActive: false, isPrimary: false, endsAt: now },
    });
  }
  if (same) {
    await tx.userAccessRole.update({
      where: { id: same.id },
      data: { isActive: true, isPrimary: true, endsAt: null, reason: "Personnel management" },
    });
  } else {
    const existing = await tx.userAccessRole.findFirst({
      where: { userId: user.id, roleId: accessRole.id, locationId },
      orderBy: { createdAt: "desc" },
    });
    if (existing) {
      await tx.userAccessRole.update({
        where: { id: existing.id },
        data: { isActive: true, isPrimary: true, startsAt: now, endsAt: null, reason: "Personnel management" },
      });
    } else {
...
export async function configureEmployeeAccessTx(
  tx: Tx,
  args: {
    employeeId: string;
    roleCode: string;
  ...
  if (args.cabinetEnabled) {
    ...
    await syncEmployeeOperationalContext(tx, {
      employeeId: employee.id,
      firstName: employee.firstName,
      lastName: employee.lastName,
      position: employee.position,
    ...
    await tx.userAccessRole.updateMany({
      where: { userId: user.id, isActive: true },
      data: { isActive: false, endsAt: now, isPrimary: false },
    });
    await tx.serviceMechanic.updateMany({ where: { employeeId: employee.id }, data: { userId: null } });
  }

  return { employee, roleCode, locationId, operational, user };
}
...
export async function configureEmployeeAccess(args: {
  ...
  const prisma = getPrisma();
  const result = await prisma.$transaction((tx) => configureEmployeeAccessTx(tx, args));
...
export async function deactivateEmployeeAccessTx(
  tx: Tx,
  employeeId: string,
  context: AccessContext,
  grantedScope: AccessScopeCode | null,
  ...
  await deactivateEmployeePlannerResources(tx, employee.id);
  if (employee.userId) {
...
export async function deactivateEmployeeAccess(
  employeeId: string,
  context: AccessContext,
  grantedScope: AccessScopeCode | null,
) {
  const prisma = getPrisma();
  const result = await prisma.$transaction((tx) => deactivateEmployeeAccessTx(tx, employeeId, context, grantedScope));
  await writeAuditEvent({
    entityType: "EmployeeProfile",
    entityId: employeeId,
