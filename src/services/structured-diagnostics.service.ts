...
export async function getStructuredDiagnostic(diagnosticRequestId: string) {
  const prisma = getPrisma(); await ensureDefaultDiagnosticTemplates();
  const diagnostic = await prisma.diagnosticRequest.findUnique({ where: { id: diagnosticRequestId }, include: { client: { select: { id: true, name: true, phone: true } }, vehicle: { select: { id: true, brand: true, model: true, year: true, plateNumber: true, vin: true, mileageKm: true } }, lead: { select: { id: true, need: true, comment: true } }, workOrder: { select: { id: true, status: true } } } });
...
export async function buildStructuredTechnicalConclusion(diagnosticRequestId: string) {
  const view = await getStructuredDiagnostic(diagnosticRequestId); const findings = view.inspections.flatMap((inspection) => inspection.sections.flatMap((section) => section.items.flatMap((item) => item.finding ? [{section:section.name,item}] : []))); if (!findings.length) return view.counts.total ? "За результатами структурованої діагностики критичних дефектів не виявлено." : null; const action:Record<string,string>={NONE:"потребує оцінки",REPLACE:"замінити",REPAIR:"ремонтувати",ADJUST:"відрегулювати",CLEAN:"очистити / обслужити",ADDITIONAL_DIAGNOSTICS:"провести додаткову діагностику"}; const urgency:Record<string,string>={INFO:"рекомендація",SOON:"найближчим часом",CRITICAL:"критично"}; return findings.map(({section,item})=>`${section} — ${item.name}: ${item.finding?.findingText||item.note||(item.state===DiagnosticCheckState.DEFECT?"виявлено дефект":"потребує уваги")}. Дія: ${action[item.finding?.action||"NONE"]}. Терміновість: ${urgency[item.finding?.urgency||"INFO"]}.`).join("\n");
}
