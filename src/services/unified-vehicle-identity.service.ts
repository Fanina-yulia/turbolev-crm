import { parseVehicleIdentityInput, type VehicleIdentityInputType } from "@/src/domain/vehicle-identity";
...
function fromPlateOnly(
  maskedIdentifier: string,
  result: GlobalVehicleLookupResponse,
): PublicVehicleIdentityContext {
  if (result.status !== "FOUND" || !result.vehicle) {
    return {
      state: "ASSISTED",
