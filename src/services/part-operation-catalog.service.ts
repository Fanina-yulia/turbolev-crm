...
const PACKAGE_RULES: Record<string, PartPackageRule> = {
  ...
  CV_JOINT: rule("CV_JOINT", "ASSEMBLY", "WHEEL", "PER_PIECE", "шт", "1 ШРУС на сторону", "Загальний ШРУС потрібно уточнити як зовнішній або внутрішній перед підбором.", { packageQuantity: 1, priceQuantity: 1 }),
  OIL_FILTER: rule("OIL_FILTER", "PIECE", "VEHICLE", "PER_PIECE", "шт", "1 масляний фільтр на автомобіль", "Масляний фільтр рахується поштучно.", { packageQuantity: 1, priceQuantity: 1 }),
  AIR_FILTER: rule("AIR_FILTER", "PIECE", "VEHICLE", "PER_PIECE", "шт", "1 повітряний фільтр на автомобіль", "Повітряний фільтр рахується поштучно.", { packageQuantity: 1, priceQuantity: 1 }),
  CABIN_FILTER: rule("CABIN_FILTER", "PIECE", "VEHICLE", "PER_PIECE", "шт", "1 салонний фільтр на автомобіль", "Салонний фільтр рахується поштучно.", { packageQuantity: 1, priceQuantity: 1 }),
...
const STATIC_OPERATIONS: Record<string, StaticOperationRow[]> = {
  ...
  CV_JOINT: [{ operationCode: "REPLACE_CV_JOINT", operationName: "Заміна ШРУСа", aliases: ["Заміна гранати"], positionRule: "SIDE", defaultQuantity: 1, source: "STATIC_PART_OPERATION_CATALOG" }],
  OIL_FILTER: [{ operationCode: "REPLACE_OIL_FILTER", operationName: "Заміна масляного фільтра", aliases: ["Заміна фільтра оливи", "Заміна масляного фільтру"], positionRule: "VEHICLE", defaultQuantity: 1, source: "STATIC_PART_OPERATION_CATALOG" }],
  AIR_FILTER: [{ operationCode: "REPLACE_AIR_FILTER", operationName: "Заміна повітряного фільтра", aliases: ["Заміна повітряного фільтру"], positionRule: "VEHICLE", defaultQuantity: 1, source: "STATIC_PART_OPERATION_CATALOG" }],
  CABIN_FILTER: [{ operationCode: "REPLACE_CABIN_FILTER", operationName: "Заміна салонного фільтра", aliases: ["Заміна фільтра салону", "Заміна салонного фільтру"], positionRule: "VEHICLE", defaultQuantity: 1, source: "STATIC_PART_OPERATION_CATALOG" }],
