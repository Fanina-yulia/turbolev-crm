...
export class ContractValidationError extends Error {
  readonly code: Extract<ApiErrorCode, "INVALID_REQUEST" | "INVALID_FILTER" | "INVALID_VIN_FORMAT" | "VEHICLE_INPUT_INSUFFICIENT">;
...
export function validateIdempotencyKey(value: unknown) {
  ...
  if (!/^[A-Za-z0-9._:-]{8,160}$/.test(result)) {
    throw new ContractValidationError("INVALID_REQUEST", "Некоректний X-Idempotency-Key.", {
      idempotencyKey: "INVALID_FORMAT",
    });
  }
  return result;
}
