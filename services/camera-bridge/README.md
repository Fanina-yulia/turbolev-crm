...
Server-to-server endpoint used by Turbo LEV CRM.
...
The bridge creates an ephemeral Neolink config with `discovery = "relay"`, starts a local RTSP bridge, captures one JPEG with FFmpeg, removes the temporary credentials file, and terminates Neolink.
...
The Reolink UID and camera password are not environment variables. They are entered in CRM. The password is stored encrypted server-side and is sent to this bridge only for a connection attempt.

## Production notes

- Put the bridge behind HTTPS/TLS.
- Set `CAMERA_BRIDGE_TOKEN`; an empty token is intended only for isolated development.
