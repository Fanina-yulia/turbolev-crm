...
type Payload = {
  ...
  editTarget?: "APPOINTMENT" | "WORK_ORDER";
  commercialSummary?: CommercialSummary | null;
};

const STYLE = `
[data-planner-estimate][data-commercial-summary="true"] {
  display: grid !important;
  gap: 5px !important;
  align-items: stretch !important;
}
[data-planner-commercial-row] {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
}
[data-planner-commercial-row] small {
  margin: 0 !important;
  color: var(--muted);
  font-size: 11px;
  font-weight: 850;
...
[data-planner-commercial-row] strong {
  font-size: 15px;
  white-space: nowrap;
}
[data-planner-commercial-row="pending"] strong {
  color: #a15c00 !important;
}
[data-planner-commercial-hint] {
...
function decorate(modal: HTMLElement, payload: Payload | null) {
  ...
  if (summary.mixedCurrency) {
    estimate.append(commercialRow("approved", "Суми", "Кілька валют"));
    const hint = document.createElement("p");
    hint.dataset.plannerCommercialHint = "true";
    hint.textContent = "Відкрийте комерційну пропозицію для розбивки за валютами.";
    estimate.append(hint);
    return;
  }

  estimate.append(commercialRow("approved", "Погоджено", money(summary.approvedAmount, summary.currency)));
  if (summary.pendingCount > 0 || (summary.pendingAmount ?? 0) > 0) {
    estimate.append(commercialRow("pending", "Очікує погодження", money(summary.pendingAmount, summary.currency)));
  }
...
export function PlannerCommercialSummaryEnhancer() {
  useEffect(() => {
    ...
    const tick = () => {
      ...
      void loadSummary(appointmentId, controller.signal)
        .then((payload) => {
          if (stopped || controller?.signal.aborted || active !== appointmentId) return;
