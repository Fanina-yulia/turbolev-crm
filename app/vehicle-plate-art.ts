...
export function plateSvgMarkup(display: string) {
  ...
  const numberMarkup = groups
    ? `<text x="66" y="101" textLength="129" lengthAdjust="spacing" ${textStyle}>${escapeXml(groups.left)}</text><text x="215" y="101" textLength="202" lengthAdjust="spacing" ${textStyle}>${escapeXml(groups.digits)}</text><text x="450" y="101" textLength="130" lengthAdjust="spacing" ${textStyle}>${escapeXml(groups.right)}</text>`
    : `<text x="306" y="101" text-anchor="middle" textLength="470" lengthAdjust="spacing" ${textStyle}>${escapeXml(display)}</text>`;
