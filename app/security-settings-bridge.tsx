...
export function SecuritySettingsBridge(){
  ...
  useEffect(()=>{
    ...
    const wire=()=>{
      ...
      host=tabs.querySelector<HTMLDivElement>('[data-security-settings-tab="true"]');
    ...
    window.addEventListener("turbolev:settings-tab",onExternalTab);
    window.addEventListener("popstate",syncUrl);
    wire();syncUrl();
    return()=>{
      observer.disconnect();
      document.removeEventListener("click",handleClick,true);
      window.removeEventListener("turbolev:settings-tab",onExternalTab);
  ...
  const activate=()=>{
    ...
    url.searchParams.set("section","settings");url.searchParams.set("settingsTab","security");
    window.history.pushState({},"",`${url.pathname}${url.search}${url.hash}`);
    window.dispatchEvent(new CustomEvent("turbolev:settings-tab",{detail:"security"}));
  };

  return <>
    {tabTarget?createPortal(<button type="button" className={`${settingsStyles.tab} ${active?settingsStyles.tabActive:""}`} onClick={activate}><span className={settingsStyles.tabIcon}>⌾</span><span><strong>Ролі та доступи</strong><small>Кабінети, дії, scope</small></span></button>,tabTarget):null}
    {active&&contentTarget?createPortal(<div style={{position:"absolute",inset:0,zIndex:40,overflow:"auto",background:"var(--bg)",padding:"20px 24px 40px"}}><SecuritySettingsPanel/></div>,contentTarget):null}
  </>;
}
