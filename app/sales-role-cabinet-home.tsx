...
import styles from "./sales-role-cabinet-home.module.css";

type SalesRole = "HEAD_OF_SALES" | "SALES";

type QueueItem = {
  lead: Lead;
  kind: "OVERDUE" | "DUE" | "NEW";
};
...
export function SalesRoleCabinetHome({ role, access }: { role: SalesRole; access: CrmAccessSnapshot }) {
  ...
  const queue = useMemo<QueueItem[]>(() => {
    const result = open.map((lead): QueueItem => ({
      lead,
      kind: isOverdue(lead, slaMinutes) ? "OVERDUE" : dueToday(lead.nextContactAt) ? "DUE" : "NEW",
    }));
