...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!created.ok) {
      return NextResponse.json({ ok: false, error: `Конфлікт: ${created.conflict.resource} уже зайнятий у цей час.`, conflict: created.conflict }, { status: 409 });
    }
    ...
    if (createdAppointmentId) {
      await prisma.serviceAppointment.delete({ where: { id: createdAppointmentId } }).catch(() => undefined);
    }
    if (error instanceof LeadNotFoundError) return NextResponse.json({ ok: false, error: error.message }, { status: 404 });
    const code = error instanceof Error ? error.message : "UNKNOWN";
    const message = code === "INVALID_TIME_RANGE" ? "Некоректний час запису." : code === "LOCATION_REQUIRED" ? "Оберіть локацію СТО." : "Не вдалося записати лід у Планувальник.";
