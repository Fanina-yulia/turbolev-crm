...
export async function PATCH(request: Request, context: { params: Promise<{ id: string; findingId: string }> }) {
  const { id: workOrderId, findingId } = await context.params;
  try {
    ...
    if (!finding || finding.workOrderId !== workOrderId) return fail("Виявлену несправність не знайдено в цьому наряді.", "FINDING_NOT_FOUND", 404);
