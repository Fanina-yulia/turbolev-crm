...
async function probe(cookie: string, label: string, path: string) {
  ...
  try {
    const response = await fetch(`${PROD}${path}`, {
      ...
      headers: { Cookie: cookie, Accept: path.startsWith("/api/") ? "application/json" : "text/html", "User-Agent": "TurboLEV-QA-Role-Audit/2026-08-21" },
    });
    const contentType = response.headers.get("content-type") || "";
    let summary: unknown = null;
    if (contentType.includes("application/json")) summary = jsonSummary(await response.json().catch(() => null));
    else {
      const text = await response.text().catch(() => "");
      summary = { titlePresent: text.includes("Turbo LEV CRM"), signInPresent: text.includes("Увійти") || text.includes("auth/sign-in"), mechanicCabinetHint: text.includes("Кабінет механіка"), bytes: text.length };
    }
    return { label, path, status: response.status, redirect: response.headers.get("location"), ms: Date.now() - started, summary };
  } catch (error) {
...
export async function GET(request: Request) {
  ...
  if (!ALLOWED_ROLES.has(role)) return NextResponse.json({ ok: false, error: "ROLE_REQUIRED" }, { status: 400 });
  ...
  const user = await prisma.user.findUnique({
    ...
    select: {
      ...
      accessRoles: { where: { isActive: true }, select: { isPrimary: true, locationId: true, role: { select: { code: true, name: true } } } },
    },
  });
  const hash = user?.employeeProfile?.crmPasswordHash || null;
  if (!user || !user.isActive || user.employeeProfile?.isActive === false || !hash) return NextResponse.json({ ok: false, error: "QA_USER_NOT_READY", role }, { status: 409 });
  ...
  return NextResponse.json({ ok: failures.length === 0, role, user: { id: user.id, name: user.name, crmLogin: user.employeeProfile?.crmLogin || null }, accessRoles: user.accessRoles, testedAt: new Date().toISOString(), failures: failures.length, results }, { headers: { "Cache-Control": "no-store" } });
}
