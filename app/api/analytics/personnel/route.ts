...
import { getPersonnelOperationalAnalytics } from "@/src/services/personnel-operational-analytics.service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const permitted = context.enforcementMode !== "ENFORCED" || hasPermission(context, PERMISSIONS.ANALYTICS_PERSONNEL_READ);
    if (!permitted) {
      return NextResponse.json({ ok: true, permitted: false, personnel: null }, { headers: { "Cache-Control": "no-store" } });
    }
    ...
    const mechanicId = request.nextUrl.searchParams.get("mechanicId")?.trim() || null;
    const postId = request.nextUrl.searchParams.get("postId")?.trim() || null;
    const personnel = await getPersonnelOperationalAnalytics({
      from,
      to,
    ...
    console.error("[analytics/personnel] failed", error);
    return NextResponse.json({ ok: false, error: "Не вдалося побудувати аналітику персоналу." }, { status: 500 });
  }
}
