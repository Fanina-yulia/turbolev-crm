...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("POST /api/personnel/owner-view-as/switch failed", error instanceof Error ? error.message : "unknown");
    return NextResponse.json({ ok: false, error: "OWNER_PREVIEW_SWITCH_FAILED" }, { status: 500 });
  }
}
