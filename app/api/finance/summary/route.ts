...
function currentKyivMonthRange(now = new Date()) {
  ...
  return {
    from: kyivDateStartUtc(year, month, 1),
...
export async function GET(request: NextRequest) {
  ...
  const pnlBySection = Object.fromEntries(
    pnlGroups.map((row) => [row.pnlSection, decimalToNumber(row._sum.amount)]),
  );
  ...
  const cashPeriod = cashPeriodGroups.reduce(
    (acc, row) => {
      const amount = decimalToNumber(row._sum.amount);
      if (row.kind === "INFLOW") acc.inflow += amount;
      if (row.kind === "OUTFLOW") acc.outflow += amount;
      if (row.flowSection === "OPERATING") {
        if (row.kind === "INFLOW") acc.operating += amount;
        if (row.kind === "OUTFLOW") acc.operating -= amount;
      }
      if (row.flowSection === "INVESTING") {
        if (row.kind === "INFLOW") acc.investing += amount;
        if (row.kind === "OUTFLOW") acc.investing -= amount;
      }
      if (row.flowSection === "FINANCING") {
        if (row.kind === "INFLOW") acc.financing += amount;
        if (row.kind === "OUTFLOW") acc.financing -= amount;
      }
      return acc;
    },
  ...
  const cashAll = Object.fromEntries(
    cashAllTimeGroups.map((row) => [row.kind, decimalToNumber(row._sum.amount)]),
  );
  ...
  return NextResponse.json(
    {
      ok: true,
      currency,
      range: { from, to, timezone: KYIV_TZ },
      hasFinancialData,
      pnl: {
      ...
      cashFlow: {
        inflow: roundMoney(cashPeriod.inflow),
        outflow: roundMoney(cashPeriod.outflow),
        net: roundMoney(cashPeriod.inflow - cashPeriod.outflow),
        operating: roundMoney(cashPeriod.operating),
        investing: roundMoney(cashPeriod.investing),
        financing: roundMoney(cashPeriod.financing),
