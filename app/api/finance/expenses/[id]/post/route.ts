...
import { FinanceExpenseError, postExpense, type ExpensePostingInput } from "@/src/services/finance-expenses.service";
...
function errorResponse(error: unknown) {
  ...
  console.error("[finance-expense-post]", error);
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await postExpense(id, body as ExpensePostingInput, user?.id || null, user?.employeeName || user?.name || user?.email || "CRM / Фінанси");
