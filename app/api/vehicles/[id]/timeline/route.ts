...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    ...
    console.error("GET /api/vehicles/[id]/timeline failed", { id, message: error instanceof Error ? error.message : "unknown" });
    return NextResponse.json({ ok: false, error: "Не вдалося завантажити хронологію автомобіля." }, { status: 500 });
  }
}
