...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const issues = await listVehicleIssues(id, scope);
    if (access.shadowBypass || access.grantedScope === "ALL") {
      return NextResponse.json({ ok: true, vehicleId: id, scope, issues }, { headers: { "Cache-Control": "no-store" } });
    }
    ...
    return NextResponse.json({ ok: true, vehicleId: id, scope, issues: scopedIssues }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
