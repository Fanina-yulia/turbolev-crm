...
import { closeManagementPlan } from "@/src/services/management-plan-workflow.service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await closeManagementPlan({ planId: id, actor, reason: typeof body.reason === "string" ? body.reason : null });
    ...
    console.error("management plan close failed", error);
    return NextResponse.json({ ok: false, error: "Не вдалося закрити тижневий план." }, { status: 500 });
  }
}
