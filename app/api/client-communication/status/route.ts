...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!clientId && phoneNormalized) {
      const direct = await prisma.client.findUnique({ where: { phoneNormalized }, select: { id: true } });
      if (direct?.id) {
        clientId = direct.id;
      } else {
        const phoneRecord = await prisma.clientPhone.findUnique({ where: { phoneNormalized }, select: { clientId: true } });
