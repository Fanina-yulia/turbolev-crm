...
export function PartsCatalog() {
  ...
  function openOrder(row: WorkOrderRow) { if (!row.id || !row.diagnosticRequest) { navigateCrm("Замовлення-наряди", { workOrderId: row.id }); return; } navigateCrm("Підбір запчастин", { diagnosticId: row.diagnosticRequest.id, workOrderId: row.id, workOrderNumber: orderLabel(row), vehicleId: row.vehicle.id, plate: row.vehicle.plateNumber || "", vin: row.vehicle.vin || "" }); }
  ...
  if (!route.diagnosticId) return <div className={styles.page}><div className={styles.menuHeader}><div><p>КОМЕРЦІЙНА ПРОПОЗИЦІЯ</p><h1>Комерційна пропозиція</h1><span>Оберіть ремонтне замовлення, щоб відкрити робочу область підбору.</span></div><span className={styles.headerBadge}>ВХІД ІЗ ГОЛОВНОГО МЕНЮ</span></div><section className={styles.orderPicker}><div className={styles.pickerHeader}><div><b>Ремонтні замовлення</b><span>Прямі ремонти без Діагностичної карти в цей розділ не потрапляють.</span></div><strong>{workOrderOptions.filter((row) => Boolean(row.diagnosticRequest)).length}</strong></div><label className={styles.orderSearch}><span>⌕</span><input value={orderSearch} onChange={(event) => setOrderSearch(event.target.value)} onKeyDown={(event) => { if (event.key !== "Enter") return; event.preventDefault(); void loadWorkOrders(); }} placeholder="Пошук за № замовлення, клієнтом, телефоном, держномером або VIN" aria-keyshortcuts="Enter"/><button type="button" onClick={() => setOrderSearch("")} disabled={!orderSearch} aria-label="Очистити пошук">×</button></label>{ordersLoading ? <div className={styles.pickerEmpty}>Завантажую ремонтні замовлення…</div> : filteredOrders.filter((row) => Boolean(row.diagnosticRequest)).length ? <div className={styles.orderList}>{filteredOrders.filter((row) => Boolean(row.diagnosticRequest)).map((row) => <button type="button" className={styles.orderRow} key={row.id} onClick={() => openOrder(row)}><span className={styles.orderNumber}>{orderLabel(row)}</span><span className={styles.orderIdentity}><b>{vehicleLabel(row)}</b><small>{row.vehicle.plateNumber || "Номер не вказаний"} · {row.client.name || "Клієнт не вказаний"}</small></span><span className={styles.orderNeed}><b>{row.statusLabel}</b><small>{row.diagnosticRequest?.status === "CONFIRMED" ? "Діагностика підтверджена" : "Діагностика в роботі"}</small></span><span className={styles.orderArrow}>→</span></button>)}</div> : <div className={styles.pickerEmpty}>{message}<button type="button" onClick={() => void loadWorkOrders()}>Оновити список</button></div>}</section></div>;
                  ...
                  : offer.compatibilityTier === "UNCONFIRMED"
                    ...
                    : offer.fitmentStatus !== "VERIFIED"
                      ...
                      : offer.fitmentExact === false || fitment?.exact === false
                        ...
                        : "Сумісність підтверджена";
            return <article
              className={`${styles.pickerOffer} ${canSelectOffer ? styles.pickerOfferClickable : ""} ${selectingOffer === key ? styles.pickerOfferPending : ""}`}
              key={key + "-" + index}
              role={canSelectOffer ? "button" : undefined}
              tabIndex={canSelectOffer ? 0 : -1}
              aria-disabled={!canSelectOffer}
              title={canSelectOffer ? "Натисніть, щоб додати деталь до кошика" : undefined}
              onClick={() => { if (canSelectOffer) void selectOffer(offer); }}
              onKeyDown={(event) => {
                if (!canSelectOffer || event.target !== event.currentTarget || (event.key !== "Enter" && event.key !== " ")) return;
