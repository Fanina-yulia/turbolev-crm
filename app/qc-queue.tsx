        ...
        return <section className={styles.lane} key={category}>
          ...
          <div className={styles.cards}>{cards.length ? cards.map((card) => <article className={styles.card} key={card.id}>
            ...
            <footer>
              ...
              {data.canWrite && card.category === "IN_PROGRESS" && <><button type="button" className={styles.pass} disabled={Boolean(busy)} onClick={() => void act(card, "PASS")}>{busy === `${card.id}:PASS` ? "…" : "Пройдено → до видачі"}</button><button type="button" className={styles.fail} disabled={Boolean(busy)} onClick={() => void act(card, "FAIL")}>{busy === `${card.id}:FAIL` ? "…" : "Не пройдено → доопрацювання"}</button></>}
              ...
              {data.canWrite && card.category === "PASSED" && card.workOrderStatus === "WAITING_QC" && <button type="button" className={styles.pass} disabled={Boolean(busy)} onClick={() => void act(card, "MOVE_PICKUP")}>Передати до видачі</button>}
            </footer>
