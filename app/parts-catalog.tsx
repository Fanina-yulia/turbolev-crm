...
export function PartsCatalog() {
  ...
  const [activeTab, setActiveTab] = useState<"all" | "originals" | "analogs">("all");
  ...
  async function loadWorkOrders() {
    ...
    try {
      ...
      const active = payload.workOrders.filter((row) => !["CLOSED", "CANCELLED"].includes(row.status) && Boolean(row.diagnosticRequest?.id)).map((row) => ({ ...row, number: numberMap.get(row.id) ?? row.number })).sort((left, right) => new Date(right.updatedAt).getTime() - new Date(left.updatedAt).getTime());
      setWorkOrderOptions(active);
      setMessage(active.length ? "Оберіть ремонтне замовлення, для якого потрібно підібрати деталі." : "Активних ремонтних замовлень із діагностичною картою не знайдено.");
  ...
  async function searchPart(queryValue = q, referenceValue = vehicleRef) {
    ...
    try {
      ...
      if (supplierResult.status === "fulfilled") { const data = await supplierResult.value.json().catch(() => null) as { offers?: SupplierOffer[]; providers?: SupplierProvider[]; configuredSuppliers?: string[]; supplierStatuses?: SupplierStatus[] } | null; setOffers(Array.isArray(data?.offers) ? data.offers : []); setSupplierProviders(Array.isArray(data?.providers) ? data.providers : []); setSupplierStatuses(Array.isArray(data?.supplierStatuses) ? data.supplierStatuses : []); setConfiguredSuppliers(Array.isArray(data?.configuredSuppliers) ? data.configuredSuppliers : []); } else { setOffers([]); setSupplierProviders([]); setSupplierStatuses([]); setConfiguredSuppliers([]); }
    } catch (error) { setParts([]); setVehicle(null); setOffers([]); setSupplierProviders([]); setSupplierStatuses([]); setConfiguredSuppliers([]); setMessage(error instanceof Error ? error.message : "Каталог тимчасово недоступний."); } finally { setBusy(false); }
  }

  useEffect(() => {
    let cancelled = false;
    const loadContext = async () => {
      if (!route.diagnosticId) { setContext(null); setVehicle(null); setRecommendedParts([]); setSelectedLines([]); setActiveFindingId(""); setOffers([]); setSupplierStatuses([]); setParts([]); void loadWorkOrders(); return; }
  ...
  const visibleOffers = useMemo(() => offers.filter((offer) => activeTab === "all" || (activeTab === "originals" ? offer.offerClass === "OEM" : offer.offerClass === "ANALOG")), [activeTab, offers]);

  function openOrder(row: WorkOrderRow) { navigateCrm("Підбір запчастин", { diagnosticId: row.diagnosticRequest.id, workOrderId: row.id, workOrderNumber: orderLabel(row), vehicleId: row.vehicle.id, plate: row.vehicle.plateNumber || "", vin: row.vehicle.vin || "" }); }
  ...
  async function selectOffer(offer: SupplierOffer) {
    ...
    try {
      const vehicleVin = normalizeVin(resolvedVin || context?.vin || (looksLikeVin(vehicleRef) ? vehicleRef : ""));
      const searchMode = vehicleVin ? "VIN" : q.trim().toUpperCase() === offer.article.trim().toUpperCase() ? "PART_NUMBER" : "TEXT";
      const response = await fetch("/api/parts-selection/select", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ diagnosticId: route.diagnosticId, findingId: activeRecommendation.findingId, supplierId: offer.supplierId, externalProductId: offer.externalProductId, article: offer.article, quantity: activeRecommendation.quantity, searchMode, vehicleVin: vehicleVin || null, manualConfirmation: !vehicleVin }) });
  ...
  if (!route.diagnosticId) return <div className={styles.page}><div className={styles.menuHeader}><div><p>РОЗДІЛ ЗАПЧАСТИН</p><h1>Підбір запчастин</h1><span>Оберіть ремонтне замовлення, щоб відкрити робочу область підбору.</span></div><span className={styles.headerBadge}>ВХІД ІЗ ГОЛОВНОГО МЕНЮ</span></div><section className={styles.orderPicker}><div className={styles.pickerHeader}><div><b>Ремонтні замовлення</b><span>Показані активні замовлення з прив’язаною Діагностичною картою.</span></div><strong>{workOrderOptions.length}</strong></div><label className={styles.orderSearch}><span>⌕</span><input value={orderSearch} onChange={(event) => setOrderSearch(event.target.value)} placeholder="Пошук за № замовлення, клієнтом, телефоном, держномером або VIN"/><button type="button" onClick={() => setOrderSearch("")} disabled={!orderSearch} aria-label="Очистити пошук">×</button></label>{ordersLoading ? <div className={styles.pickerEmpty}>Завантажую ремонтні замовлення…</div> : filteredOrders.length ? <div className={styles.orderList}>{filteredOrders.map((row) => <button type="button" className={styles.orderRow} key={row.id} onClick={() => openOrder(row)}><span className={styles.orderNumber}>{orderLabel(row)}</span><span className={styles.orderIdentity}><b>{vehicleLabel(row)}</b><small>{row.vehicle.plateNumber || "Номер не вказаний"} · {row.client.name || "Клієнт не вказаний"}</small></span><span className={styles.orderNeed}><b>{row.statusLabel}</b><small>{row.diagnosticRequest.status === "CONFIRMED" ? "Діагностика підтверджена" : "Діагностика в роботі"}</small></span><span className={styles.orderArrow}>→</span></button>)}</div> : <div className={styles.pickerEmpty}>{message}<button type="button" onClick={() => void loadWorkOrders()}>Оновити список</button></div>}</section></div>;
  ...
  return <div className={styles.page}><header className={styles.contextHeader}><button type="button" className={styles.backButton} onClick={() => navigateCrm("Підбір запчастин", {})} aria-label="Повернутися до вибору замовлення">←</button><div className={styles.orderContext}><small>Ремонтне замовлення</small><b>{context.orderNumber}</b></div><div className={styles.contextItem}><small>Клієнт</small><b>{context.clientName}</b></div><div className={styles.contextItem}><small>Телефон</small><b>{context.clientPhone}</b></div><div className={styles.contextItem}><small>Автомобіль</small><b>{context.vehicleName}</b><span>{context.plateNumber || "Номер не вказаний"}</span></div><div className={styles.contextItem}><small>Пробіг</small><b>{formatMileage(context.mileageKm)}</b></div><div className={styles.contextStatus}><small>Статус ЗН</small><b className={statusTone(context.statusCode)}><i/> {context.statusLabel}</b></div></header><div className={styles.columns}>
    ...
    <section className={`${styles.column} ${styles.searchColumn}`}><div className={styles.columnTitle}><span className={styles.step}>2</span><div><b>Крок 2 · Пошук</b><small>Оригінали та аналоги від підключених постачальників</small></div></div><div className={styles.searchFields}><label><span>Пошук за VIN-кодом</span><div className={styles.inputWithIcon}><input value={vehicleRef} onChange={(event) => { setVehicleRef(event.target.value.toUpperCase()); setResolvedPlate(null); setResolvedVin(""); }} placeholder="VIN або держномер"/><button type="button" onClick={() => { setVehicleRef(""); setResolvedVin(""); }} disabled={!vehicleRef}>×</button></div><small>{referenceHint}</small></label><label><span>Пошук за номером деталі</span><div className={styles.inputWithIcon}><input value={q} onChange={(event) => setQ(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void searchPart(); }} placeholder="Артикул або назва деталі"/><button type="button" onClick={() => void searchPart()} aria-label="Знайти деталь">⌕</button></div><small>{activeRecommendation ? `Для потреби: ${activeRecommendation.name}` : "Оберіть позицію зліва"}</small></label></div><div className={styles.tabs}><button type="button" className={activeTab === "all" ? styles.tabActive : ""} onClick={() => setActiveTab("all")}>Усі <span>{offers.length}</span></button><button type="button" className={activeTab === "originals" ? styles.tabActive : ""} onClick={() => setActiveTab("originals")}>Оригінал / OE <span>{offers.filter((offer) => offer.offerClass === "OEM").length}</span></button><button type="button" className={activeTab === "analogs" ? styles.tabActive : ""} onClick={() => setActiveTab("analogs")}>Аналоги <span>{offers.filter((offer) => offer.offerClass === "ANALOG").length}</span></button></div><div className={styles.apiLine}><span>{configuredSuppliers.length} доступи налаштовано</span><small>{supplierStatuses.length ? `${supplierStatuses.length} постачальники додані · ${supplierProviders.filter((provider) => provider.ok).length} відповіли на цей пошук` : "Статус постачальників ще не отримано"}</small></div>{!configuredSuppliers.length ? <div className={styles.emptyState}><b>Постачальники додані, але API не налаштовані</b><span>{supplierStatuses.length ? `У довіднику: ${supplierStatuses.map((supplier) => supplier.name).join(", ")}.` : "Перевірте BM Parts та Юнік Трейд."}</span><button type="button" onClick={() => navigateCrm("Налаштування", { settingsTab: "integrations" })}>Відкрити інтеграції</button></div> : null}{busy ? <div className={styles.emptyState}><b>Шукаю пропозиції…</b><span>Перевіряю VIN, артикул та доступні складські залишки.</span></div> : null}{!busy && configuredSuppliers.length > 0 && !offers.length ? <div className={styles.emptyState}><b>Пропозицій не знайдено</b><span>Змініть назву або артикул деталі та повторіть пошук.</span></div> : null}{!busy && offers.length > 0 && !visibleOffers.length ? <div className={styles.emptyState}><b>Немає підтверджених позицій у цій вкладці</b><span>Постачальник не передав ознаку «оригінал/аналог». Перегляньте вкладку «Усі» та підтвердьте класифікацію перед погодженням.</span></div> : null}<div className={styles.offerList}>{visibleOffers.map((offer, index) => { const key = `${offer.supplierId}:${offer.externalProductId || offer.article}`; const classification = offer.offerClass === "OEM" ? "Оригінал / OE" : offer.offerClass === "ANALOG" ? "Аналог" : "Класифікація потребує підтвердження"; return <article className={styles.offerCard} key={`${key}-${index}`}><div className={styles.offerMain}><div className={styles.offerTitle}><b>{offer.name}</b><small>Артикул {offer.article}</small></div><span className={styles.favorite} title={classification}>☆</span></div><div className={styles.offerBody}><div className={styles.offerSupplier}><b>{offer.supplierName}</b><span>{offer.brand || "Каталог постачальника"}</span><em>{classification}</em></div><div className={styles.offerMetric}><small>Ціна постачальника</small><b>{formatMoney(offer.purchasePrice, offer.currency)}</b><small>Ціна для клієнта</small><b className={styles.sellPrice}>{formatMoney(offer.sellPrice, offer.currency)}</b></div><div className={styles.offerMetric}><small>В наявності</small><b className={offer.available ? styles.available : styles.unavailable}>{offer.available ? "Так" : "Уточнити"}</b><small>{offer.stock.length ? `Склад: ${offer.stock[0].warehouse}` : "Склад не вказаний"}</small></div><button type="button" className={styles.addButton} disabled={!offer.available || offer.purchasePrice == null || selectingOffer === key || !activeRecommendation} onClick={() => void selectOffer(offer)}>{selectingOffer === key ? "Зберігаю…" : "Додати"}</button></div></article>; })}</div><div className={styles.searchFooter}><span>Показано {visibleOffers.length} із {offers.length} позицій</span><button type="button" onClick={() => setActiveTab(activeTab === "all" ? "originals" : "all")} disabled={!offers.length}>{activeTab === "all" ? "Показати лише оригінали →" : "Показати всі →"}</button></div>{providerErrors.length ? <div className={styles.warning}>Не всі API відповіли: {providerErrors.map((provider) => `${provider.id}${provider.message ? ` — ${provider.message}` : ""}`).join("; ")}</div> : null}<div className={styles.policyNote}>{message}</div></section>
