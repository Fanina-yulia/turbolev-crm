...
export function PartsCatalog() {
  ...
  function selectDiagramItem(findingId: string) {
    const item = recommendedParts.find((recommendation) => recommendation.findingId === findingId);
    if (!item) return;
    setActiveFindingId(item.findingId);
    setQ(item.name);
    setOffers([]);
    void searchPart(item.name, vehicleRef);
  }
  ...
  const diagramRecommendations: PartsDiagramRecommendation[] = recommendedParts.map(({ findingId, name, position, quantity, action, urgency }) => ({ findingId, name, position, quantity, action, urgency }));
