...
export function PartsCatalogSettingsPanel() {
  ...
  async function post(action: string, extra: Record<string, unknown> = {}) {
    ...
    try {
      ...
      const body = await response.json() as { ok?: boolean; error?: string; result?: { kits?: number; operations?: { articles?: number; operations?: number; linkedOperations?: number }; terminology?: { articleCount?: number; aliasCount?: number } } };
      if (!response.ok || !body.ok) throw new Error(body.error || "Операцію не виконано");
      setMessage(action === "SEED_REPAIR_KITS" ? `Комплекти синхронізовано: ${body.result?.kits || 0}.` : action === "SEED_PART_OPERATION_CATALOG" ? `Зв’язки деталей із роботами синхронізовано: ${body.result?.operations?.operations || 0}.` : action === "SEED_FULL_PART_CATALOG" ? `Повний каталог оновлено: ${body.result?.operations?.articles || 0} деталей і ${body.result?.operations?.operations || 0} зв’язків із роботами.` : "Зміни збережено та відправлено на перевірку.");
      await load();
  ...
  return <div className={styles.page}>
    ...
    </div><div className={styles.actions}><button disabled={saving || !form.code || !form.name} onClick={() => void post("UPSERT_PART", form)}>Створити на перевірку</button><button className={styles.secondary} disabled={saving} onClick={() => void post("SEED_REPAIR_KITS")}>Створити комплекти ремонту</button><button className={styles.secondary} disabled={saving} onClick={() => void post("SEED_FULL_PART_CATALOG")}>Синхронізувати повний каталог</button></div></div>
