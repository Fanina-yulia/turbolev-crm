...
type SupportContact = { name: string | null; phone: string | null } | null;
...
export function MechanicStandaloneCabinet({ userName }: { userName?: string | null }) {
  ...
  const [supportContact, setSupportContact] = useState<SupportContact>(null);
  ...
  const loadSupport = useCallback(async () => {
    const response = await fetch("/api/cabinet/mechanic/support", { cache: "no-store", credentials: "include" });
    const body = await response.json().catch(() => null);
    if (response.ok && body?.ok) setSupportContact(body.advisor ?? null);
  }, []);
  ...
  useEffect(() => {
    ...
    void Promise.all([loadHome(), loadTasks(), loadDiagnostics(), loadNotifications(), loadSupport()]).catch((cause) => setError(cause instanceof Error ? cause.message : "Не вдалося завантажити кабінет"));
  }, [loadDiagnostics, loadHome, loadNotifications, loadSupport, loadTasks]);
  ...
  function callAdvisor() {
    const phone = supportContact?.phone?.replace(/[^+\d]/g, "");
    if (!phone) { setError("Телефон сервіс-менеджера ще не вказаний у Персоналі."); return; }
  ...
  return <div className={styles.app} data-theme-choice={themeChoice}>
