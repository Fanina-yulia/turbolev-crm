...
function money(value: number) {
  return new Intl.NumberFormat("uk-UA", { style: "currency", currency: "UAH", maximumFractionDigits: 2 }).format(value || 0);
}
...
export function FinanceExpensesV2({ from, to, locationId, categories, accounts, onCreate, onChanged }: Props) {
  ...
  return <section className={styles.panel}>
    <div className={styles.panelHeader}><div><span className={styles.eyebrow}>EXPENSE CONTROL</span><h2>Витрати</h2><p>Первинні документи, погодження, часткові оплати, розподіл по категоріях і сторно.</p></div><button type="button" className={styles.primaryButton} onClick={onCreate}>+ Додати витрату</button></div>
