        ...
        <tbody>
          {loading && !rows.length ? <tr><td colSpan={7}><div className={styles.empty}>Завантажую журнал робіт…</div></td></tr> : !rows.length ? <tr><td colSpan={7}><div className={styles.empty}>За вибраними фільтрами робіт не знайдено.</div></td></tr> : rows.map((row) => <Fragment key={row.id}>
            <tr>
              ...
              <td><span className={styles.status}>{row.statusLabel}</span><div style={{ marginTop: 4, fontSize: 11, color: "var(--muted)" }}>{stateDate(row)}</div></td>
              <td>{row.payment ? <><strong className={paymentClass(row.payment.code)}>{row.payment.label}</strong><div style={{ marginTop: 3, fontSize: 11, color: "var(--muted)" }}>{row.payment.code === "PAID" ? money(row.payment.paid) : row.payment.outstanding > 0 ? `Борг ${money(row.payment.outstanding)}` : money(row.payment.total)}</div></> : <span className={styles.muted}>—</span>}</td>
