...
export function StationOverview(){
  ...
  const cars=useMemo<AttentionCar[]>(()=>data?[...data.attention].sort((a,b)=>urgency(a.status)-urgency(b.status)||new Date(a.plannedStartAt).getTime()-new Date(b.plannedStartAt).getTime()).map((item)=>{const car=parseVehicle(item.vehicle);const route=statusRoute[item.status]||{section:"Авто" as CrmSectionLabel,filter:"",label:"Автомобіль"};return{id:item.id,plate:item.plate,brand:car.brand,model:car.model,year:car.year,status:statusLabels[item.status]||item.status,action:statusAction[item.status]||item.problem||"Відкрити картку",owner:item.mechanic||item.post||"Не призначено",problem:item.problem,plannedStartAt:item.plannedStartAt,tone:tone(item.status),section:route.section,filter:route.filter,filterLabel:`${route.label} · ${item.plate}`};}):[],[data]);
  ...
  return <>
    ...
    <section className="gridTwo"><WorkOrderCockpit cars={cars} onAll={()=>navigate("Авто","attention","Потребують уваги")} onOpen={car=>navigate(car.section as CrmSectionLabel,car.filter,car.filterLabel)}/><aside className="panel blockers"><div className="sectionHead"><div><p className="eyebrow">БЛОКЕРИ</p><h2>Що стопорить потік</h2></div></div><div className="blockerList">{blockers.map(item=><button type="button" className="blocker blockerAction" key={item.name} onClick={()=>navigate(item.section,item.filter,item.filterLabel)}><b>{item.name}</b><strong>{item.value}</strong><span>{item.sub}</span><em>{item.section} →</em></button>)}</div><div className="blockerHint">Блокер — це не просто статус. Тут мають залишатися тільки авто, де потік реально зупинено і потрібне рішення людини.</div></aside></section>
  </>;
}
