  ...
  return <div className={styles.page}>
    ...
    {data && !data.emptyScope && tab === "overview" && <>
      <section className={styles.kpis}>
        ...
        <KpiCard label="Середній чек" value={data.permissions?.financial ? money(kpi?.averageCheck) : "Обмежено"} hint={`${kpi?.closedWorkOrders || 0} закритих ЗН`} current={kpi?.averageCheck} previous={previous?.averageCheck} onClick={() => navigateCrm("Комерційна пропозиція", { status: "CLOSED" })} />
