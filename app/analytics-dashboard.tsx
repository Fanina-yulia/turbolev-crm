...
export function AnalyticsDashboard() {
  ...
  const funnelStages = useMemo(() => {
    ...
    stages.push({ key: "workOrder", label: "Створено КП", count: funnel.workOrderLinked, conversion: funnel.diagnosticsToWorkOrderPct, route: () => navigateCrm("Комерційна пропозиція") });
  ...
  return <div className={styles.page}>
    ...
    {data && !data.emptyScope && tab === "overview" && <>
      <section className={styles.kpis}>
        <KpiCard label="Виручка" value={data.permissions?.financial ? money(kpi?.grossRevenue) : "Обмежено"} hint="фіналізовані КП" current={kpi?.grossRevenue} previous={previous?.grossRevenue} onClick={() => setTab("finance")} />
        ...
        <KpiCard label="Середній чек" value={data.permissions?.financial ? money(kpi?.averageCheck) : "Обмежено"} hint={`${kpi?.closedWorkOrders || 0} закритих КП`} current={kpi?.averageCheck} previous={previous?.averageCheck} onClick={() => navigateCrm("Комерційна пропозиція", { status: "CLOSED" })} />
        ...
        <KpiCard label="Готово до видачі" value={number(kpi?.readyNow)} hint="можна завершувати" onClick={() => navigateCrm("Комерційна пропозиція", { status: "READY_FOR_PICKUP" })} tone="good" />
      ...
      <div className={styles.twoColumns}>
        <section className={styles.panel}><PanelTitle eyebrow="ДИНАМІКА" title="Закриті КП за днями" right={<span>{from} — {to}</span>} />
          {(data.trend?.length || 0) === 0 ? <p className={styles.note}>У періоді ще немає закритих КП.</p> : <div className={styles.trend}>{data.trend!.map((row) => <div className={styles.trendRow} key={row.date}><time>{dateLabel(row.date)}</time><div className={styles.barTrack}><i style={{ width: `${Math.max(3, (Math.max(row.closed, (row.revenue || 0) / 1000) / maxTrend) * 100)}%` }} /></div><b>{row.closed} КП</b>{data.permissions?.financial && <span>{money(row.revenue)}</span>}</div>)}</div>}
        </section>
    ...
    {tab === "diagnostics" && <>
      ...
      {diagnostics?.permitted && diagnostics.diagnostics && (() => {
        ...
        return <>
          <section className={styles.kpisSix}><KpiCard label="Діагностик" value={number(d.created)} hint="створено за період" onClick={() => navigateCrm("Діагностика")} /><KpiCard label="Завершено" value={number(d.completed)} hint={`${d.confirmed} підтверджено`} /><KpiCard label="Зауважень" value={number(issueCount)} hint={`${percent(issuePct)} перевірених пунктів`} tone={issueCount ? "warn" : "good"} /><KpiCard label="Критичних" value={number(d.checks.critical)} hint="urgency CRITICAL" tone={d.checks.critical ? "danger" : "good"} /><KpiCard label="Діагностика → КП" value={percent(d.conversionPct)} hint={`${d.convertedToWorkOrder} КП створено`} onClick={() => navigateCrm("Комерційна пропозиція")} /><KpiCard label="Середній час" value={durationLabel(d.averageInspectionMinutes)} hint="старт → завершення" /></section>
    ...
    {tab === "finance" && <>
      ...
      {finance?.permitted && finance.finance && (() => { const f = finance.finance; return <>
        ...
        <div className={styles.twoColumnsWide}>
          ...
          <section className={styles.panel}><PanelTitle eyebrow="КП · ACTUAL" title="Економіка замовлень" /><div className={styles.financeCards}><div><small>Фіналізовано КП</small><strong>{f.orderEconomics.finalizedOrders}</strong></div><div><small>Виручка</small><strong>{money(f.orderEconomics.grossRevenue)}</strong></div><div><small>Середній чек</small><strong>{money(f.orderEconomics.averageCheck)}</strong></div><div><small>Валова маржа</small><strong>{percent(f.orderEconomics.grossMarginPct)}</strong></div></div><p className={styles.note}>Цей блок рахується тільки з ACTUAL finance snapshot закритих КП вибраного періоду.</p></section>
        </div>
