...
const statusMeta:Record<WorkflowState,{label:string;note:string}>={
  ...
  IN_PROGRESS:{label:"Діагностика триває",note:"Автомеханік формує структурований технічний висновок"},
  SUBMITTED:{label:"На перевірці",note:"Механік завершив діагностику та передав сервіс-менеджеру"},
...
const filters:Array<{value:Filter;label:string}>=[
  {value:"ALL",label:"Усі"},{value:"PENDING",label:"Очікують"},{value:"IN_PROGRESS",label:"В роботі"},{value:"SUBMITTED",label:"На перевірці"},{value:"RETURNED",label:"Повернені"},{value:"CONFIRMED",label:"Підтверджені"},{value:"CANCELLED",label:"Скасовані"},
];
...
export function Diagnostics(){
  ...
  useEffect(()=>{void load();},[]);
  const visible=useMemo(()=>filter==="ALL"?rows:rows.filter(item=>filter==="SUBMITTED"||filter==="RETURNED"?workflowState(item)===filter:item.status===filter),[rows,filter]);
  const selected=rows.find(item=>item.id===selectedId)??null;
  useEffect(()=>{setConclusion(selected?.technicalConclusion??"");setMessage("");},[selectedId,selected?.technicalConclusion]);
  const counts=useMemo(()=>({pending:rows.filter(x=>x.status==="PENDING").length,inProgress:rows.filter(x=>x.status==="IN_PROGRESS"&&workflowState(x)==="IN_PROGRESS").length,submitted:rows.filter(x=>workflowState(x)==="SUBMITTED").length,confirmed:rows.filter(x=>x.status==="CONFIRMED").length,withWorkOrder:rows.filter(x=>Boolean(x.workOrder)).length}),[rows]);

  async function transition(status:DiagnosticStatus){if(!selected)return;setSaving(true);setError("");setMessage("");try{const response=await fetch(`/api/diagnostics/${selected.id}`,{method:"PATCH",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({status,technicalConclusion:conclusion})});const data=await response.json() as ApiResponse;if(!response.ok||!data.ok||!data.diagnostic)throw new Error(data.message||data.error||"Не вдалося змінити статус");await load();if(status==="CONFIRMED")setMessage(data.diagnostic.workOrder?`Діагностику підтверджено. WorkOrder ${data.diagnostic.workOrder.id.slice(-8)} створено/прив’язано.`:"Діагностику підтверджено.");else setMessage("Статус діагностики оновлено.");}catch(e){setError(e instanceof Error?e.message:"Помилка зміни статусу");}finally{setSaving(false);}}

  const filterCount=(value:Filter)=>value==="ALL"?rows.length:value==="SUBMITTED"||value==="RETURNED"?rows.filter(row=>workflowState(row)===value).length:rows.filter(row=>row.status===value).length;

  return <div className={styles.page}>
    <header className={styles.head}><div><p className={styles.eyebrow}>СЕРВІС · HARD GATE #1</p><h1>Діагностика</h1><p>Механік веде структурований чекліст і передає результат на перевірку. WorkOrder створюється тільки після підтвердження сервіс-менеджером.</p></div><button className={styles.refresh} onClick={()=>void load()} disabled={loading}>{loading?"Оновлюю…":"Оновити"}</button></header>
    <section className={styles.kpis}><div><span>Очікують</span><strong>{counts.pending}</strong></div><div><span>В роботі</span><strong>{counts.inProgress}</strong></div><div><span>На перевірці</span><strong>{counts.submitted}</strong></div><div><span>Підтверджено / WO</span><strong>{counts.confirmed}/{counts.withWorkOrder}</strong></div></section>
    ...
    <div className={styles.layout}>
      ...
      <aside className={styles.detail}>{selected?<><div className={styles.detailHead}><div><span className={`${styles.status} ${stateClass(selected)}`}>{statusMeta[workflowState(selected)].label}</span><h2>{vehicleName(selected)}</h2><p>{selected.vehicle.plateNumber||"Без держномера"} · {selected.vehicle.vin||"VIN не вказано"}</p></div></div>
        ...
        {["SUBMITTED","CONFIRMED"].includes(selected.reviewState||"")&&<DiagnosticReportSharePanel diagnosticId={selected.id} reviewState={selected.reviewState||workflowState(selected)} workOrder={selected.workOrder}/>} 
