...
type WorkflowState=DiagnosticStatus|"SUBMITTED"|"REVIEWING"|"RETURNED"|"CARD_SENT";
type Diagnostic={
  id:string;status:DiagnosticStatus;workflowState?:WorkflowState;reviewState?:string;reviewerUserId?:string|null;technicalConclusion:string|null;confirmedAt:string|null;createdAt:string;updatedAt:string;
...
type Filter="ALL"|DiagnosticStatus|"SUBMITTED"|"RETURNED"|"CARD_SENT"|"MANAGER_REVIEW";

const statusMeta:Record<WorkflowState,{label:string;note:string}>={
  PENDING:{label:"Заплановано",note:"Авто ще не підтверджене механіком скануванням"},
  IN_PROGRESS:{label:"В роботі",note:"Механік проводить діагностику"},
  SUBMITTED:{label:"Завершена діагностика",note:"Механік завершив діагностику; очікується сервіс-менеджер"},
  REVIEWING:{label:"На перевірці менеджера",note:"Сервіс-менеджер перевіряє результат"},
  RETURNED:{label:"В роботі",note:"Повернено механіку на уточнення"},
  CONFIRMED:{label:"На перевірці менеджера",note:"Діагностику зафіксовано; ДК ще не відправлена"},
  CARD_SENT:{label:"Очікує рішення клієнта",note:"Діагностична карта надіслана і доступна у кабінеті власника"},
  CANCELLED:{label:"Скасовано",note:"Діагностику закрито"},
};
const filters:Array<{value:Filter;label:string}>=[
  {value:"ALL",label:"Усі"},
  {value:"PENDING",label:"Заплановано"},
  {value:"IN_PROGRESS",label:"В роботі"},
  {value:"SUBMITTED",label:"Завершена діагностика"},
  {value:"MANAGER_REVIEW",label:"На перевірці менеджера"},
  {value:"RETURNED",label:"Повернено механіку"},
  {value:"CARD_SENT",label:"Очікує рішення клієнта"},
...
function stateClass(row:Diagnostic){const state=workflowState(row);if(state==="SUBMITTED"||state==="REVIEWING")return styles.IN_PROGRESS;if(state==="RETURNED")return styles.PENDING;if(state==="CARD_SENT")return styles.CONFIRMED;return styles[row.status];}
function managerReview(row:Diagnostic){const state=workflowState(row);return state==="REVIEWING"||state==="CONFIRMED";}
...
export function Diagnostics(){
  ...
  const visible=useMemo(()=>filter==="ALL"?rows:rows.filter(item=>{
    ...
    if(filter==="MANAGER_REVIEW")return managerReview(item);
  ...
  const counts=useMemo(()=>({planned:rows.filter(x=>x.status==="PENDING").length,inProgress:rows.filter(x=>["IN_PROGRESS","RETURNED"].includes(workflowState(x))).length,submitted:rows.filter(x=>workflowState(x)==="SUBMITTED").length,reviewing:rows.filter(managerReview).length,clientDecision:rows.filter(x=>workflowState(x)==="CARD_SENT").length}),[rows]);

  async function transition(status:DiagnosticStatus){if(!selected)return;setSaving(true);setError("");setMessage("");try{const response=await fetch(`/api/diagnostics/${selected.id}`,{method:"PATCH",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({status,technicalConclusion:conclusion})});const data=await response.json() as ApiResponse;if(!response.ok||!data.ok||!data.diagnostic)throw new Error(data.message||data.error||"Не вдалося змінити статус");await load();setMessage(status==="CONFIRMED"?"Діагностику зафіксовано. Після відправки ДК авто перейде у «Очікує рішення клієнта».":"Статус діагностики оновлено.");}catch(e){setError(e instanceof Error?e.message:"Помилка зміни статусу");}finally{setSaving(false);}}

  const filterCount=(value:Filter)=>{
    if(value==="ALL")return rows.length;
    if(value==="MANAGER_REVIEW")return rows.filter(managerReview).length;
  ...
  return <div className={styles.page}>
    <header className={styles.head}><div><p className={styles.eyebrow}>СЕРВІС · ДІАГНОСТИКА</p><h1>Діагностика</h1><p>Еталонний шлях: Заплановано → В роботі → Завершена діагностика → На перевірці менеджера → Очікує рішення клієнта.</p></div><button className={styles.refresh} onClick={()=>void load()} disabled={loading}>{loading?"Оновлюю…":"Оновити"}</button></header>
    <section className={styles.kpis}><div><span>Заплановано</span><strong>{counts.planned}</strong></div><div><span>В роботі</span><strong>{counts.inProgress}</strong></div><div><span>Завершена діагностика</span><strong>{counts.submitted}</strong></div><div><span>На перевірці</span><strong>{counts.reviewing}</strong></div><div><span>Очікує клієнта</span><strong>{counts.clientDecision}</strong></div></section>
    ...
    <div className={styles.layout}>
      ...
      <aside className={styles.detail}>{selected?<><div className={styles.detailHead}><div><span className={`${styles.status} ${stateClass(selected)}`}>{statusMeta[workflowState(selected)].label}</span><h2>{vehicleName(selected)}</h2><p>{selected.vehicle.plateNumber||"Без держномера"} · {selected.vehicle.vin||"VIN не вказано"}</p><small>{statusMeta[workflowState(selected)].note}</small></div></div>
        ...
        {selected.structured?.inspections?<StructuredDiagnosticReviewPanel diagnosticId={selected.id} onChanged={load}/>:<div className={styles.actions}>{selected.status==="PENDING"&&<><button className={styles.primary} disabled={saving} onClick={()=>void transition("IN_PROGRESS")}>Почати стару діагностику</button><button className={styles.secondary} disabled={saving} onClick={()=>void transition("CANCELLED")}>Скасувати</button></>}{selected.status==="IN_PROGRESS"&&<><button className={styles.primary} disabled={saving||!conclusion.trim()} onClick={()=>void transition("CONFIRMED")}>{saving?"Зберігаю…":"Зафіксувати діагностику"}</button><button className={styles.secondary} disabled={saving} onClick={()=>void transition("CANCELLED")}>Скасувати</button></>}{selected.status==="CONFIRMED"&&<span className={styles.lockNote}>✓ Діагностику зафіксовано. Після відправки ДК основний статус буде «Очікує рішення клієнта».</span>}{selected.status==="CANCELLED"&&<span className={styles.lockNote}>Діагностику закрито. Для нового огляду потрібна нова заявка на діагностику.</span>}</div>}
