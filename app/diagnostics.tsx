  ...
  return <div className={styles.page}>
    ...
    <div className={`${styles.layout} ${routeDiagnosticId ? styles.detailOnly : styles.registryOnly}`}>
      ...
      <aside className={styles.detail}>{selected ? <>
        ...
        {!selected.structured?.inspections && <label className={styles.conclusion}><span>Технічний висновок</span><textarea rows={8} value={conclusion} disabled={selected.status === "CANCELLED" || selected.status === "CONFIRMED"} placeholder="Опишіть підтверджені дефекти, результати перевірки та рекомендовані роботи…" onChange={(event) => setConclusion(event.target.value)} /><small>Для старої діагностики заповніть висновок вручну.</small></label>}
