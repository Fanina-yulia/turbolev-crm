...
type Props = { clientId?: string | null; vehicleId?: string | null };
...
export function ClientCommunicationActions({clientId,vehicleId}:Props){
  const[state,setState]=useState<State|null>(null);
  const[busy,setBusy]=useState("");
  const[toast,setToast]=useState("");
  const query=useMemo(()=>{const p=new URLSearchParams();if(clientId)p.set("clientId",clientId);if(vehicleId)p.set("vehicleId",vehicleId);return p.toString()},[clientId,vehicleId]);
  ...
  return <><div className={styles.actions} aria-label="Швидкий зв’язок із клієнтом">
    ...
    <button type="button" className={`${styles.action} ${state?.phoneChannel?.active?styles.activePhone:""} ${busy==="phone"?styles.busy:""}`} disabled={loading||!state?.phoneChannel?.available||busy==="phone"} onClick={e=>{e.stopPropagation();void phone()}} title="Зателефонувати через Binotel" aria-label="Зателефонувати через Binotel"><PhoneIcon/></button>
